...
TeamAWin::TeamAWin() : GameStateBase(StateType::STATE_TEAMAWIN), m_time(0.0f)
{
}
...
void TeamAWin::Init()
{
	...
	// background
	texture = ResourceManagers::GetInstance()->GetTexture("TeamAWin.tga");
	...
	texture = ResourceManagers::GetInstance()->GetTexture("animation2.tga");
	std::shared_ptr<SpriteAnimation> count = std::make_shared<SpriteAnimation>(model, shader, texture, 4, 2, 0, 0.1f);
	count->Set2DPosition(Globals::screenWidth / 1.8f, Globals::screenHeight / 1.27f);
...
void TeamAWin::Resume()
{
}


void TeamAWin::HandleEvents()
{
}

void TeamAWin::HandleKeyEvents(int key, bool bIsPressed)
{
}

void TeamAWin::HandleTouchEvents(int x, int y, bool bIsPressed)
{
...
void TeamAWin::HandleMouseMoveEvents(int x, int y)
{
}

void TeamAWin::Update(float deltaTime)
{
